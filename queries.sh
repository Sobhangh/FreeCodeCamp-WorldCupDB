...
echo "$($PSQL "SELECT t.name FROM games g INNER JOIN teams t ON g.winner_id=t.team_id OR g.opponent_id=t.team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY t.name")"
#"$($PSQL "SELECT  FROM games WHERE")"
